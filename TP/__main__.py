...
if __name__ == "__main__":
    ...
    for i in range(len(files)):
        ...
        for j in range(i + 1, len(files)):
            ...
            jaccard_score = jaccard(
                indexA.copy(), files[filenames[j]], k
            )  # Puisque l'index sera modifié dans la fonction, on doit créer une copie avec index.copy()
    ...
    jaccard_csv = pd.DataFrame(jaccard_matrix, index=filenames, columns=filenames)
    jaccard_csv.to_csv("jac_results.csv")
